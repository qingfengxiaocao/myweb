					    	...
					    	<tbody>
					    		<?php
									...
									function news($pageNum =1, $pageSize)
									{
										...
										$rs = "select email,jiaoyie,yue,jiaoyishijian,jiaoyiyuanyin from zhangben where email='{phpCAS::getUser()}' order by jiaoyishijian desc limit " . (($pageNum - 1) * $pageSize) . "," . $pageSize;
									...
									function allNews()
									{
									    ...
									    mysqli_select_db($coon, "web");
									    mysqli_set_charset($coon, "utf8");
									    $rs = "select count(*) num from zhangben where email='{phpCAS::getUser()}'"; //可以显示出总页数
									    $r = mysqli_query($coon, $rs);

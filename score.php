...
<script>
    ...
    var maxGeneration = 3 ;   // 最多分支的次数
    var branchArray = null ;  // 树干的集合
    var flowers = [];         // 花的集合
	$.ajax({
			url: "http://"+address+"houtai/score.php",
			type: 'post',
			data: "key=yuan&email="+"<?php echo $email;?>",		
			async: true,
			success: function(data) {
				if(data != ''){
					...
					if(myresult >= 100 && myresult <= 300)
					{
						maxGeneration = 4;
					}
					if(myresult >= 300 && myresult <= 500)
					{
						maxGeneration = 5;
					}
					if(myresult >= 500)
